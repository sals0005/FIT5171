...
//    }
    ...
    public void shouldDeleteLaunchSuccessfully() {
        Launch launchT = new Launch();
        launchT.setLaunchDate(LocalDate.of(2017, 1, 1));
        launchT.setLaunchVehicle(rocket);
        launchT.setLaunchSite("VAFBA");
        launchT.setOrbit("LEO");
        dao.createOrUpdate(launchT);

        Collection<Launch> launches = dao.loadAll(Launch.class);
        assertFalse(launches.isEmpty());
        dao.delete(launchT);
        Collection<Launch> l = dao.loadAll(Launch.class);
        assertFalse(l.contains(launchT));
