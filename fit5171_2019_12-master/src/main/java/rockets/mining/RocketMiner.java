...
public class RocketMiner {
    ...
    public List<Rocket> mostLaunchedRockets(int k) {
        ...
        List<Rocket> sortedRockets = Util.getSortedRocketsByLaunches(launchList);
    ...
    public List<Payload> heaviestPayload(int k) {
        logger.info("find most expensive " + k + " launches");
        Collection<Payload> payloads = dao.loadAll(Payload.class);
        Comparator<Payload> payloadWeight = (a, b) -> -a.getWeight().compareTo(b.getWeight());
        return payloads.stream().sorted(payloadWeight).limit(k).collect(Collectors.toList());
    }
    /**
     * TODO: to be implemented & tested!
     * <p>
    ...
    public List<LaunchServiceProvider> highestRevenueLaunchServiceProviders(int k, int year) {
        ...
        Map<LaunchServiceProvider, BigDecimal> mapByLsp = Util.getRevenuePerLspInYear(launches,year);
        // GET SORTED LIST OF LSPs
        List<LaunchServiceProvider> sortedLsps = Util.getSortedLspByRevenue(mapByLsp);
        int i = 0;
