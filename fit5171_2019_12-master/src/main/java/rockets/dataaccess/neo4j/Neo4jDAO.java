...
public class Neo4jDAO implements DAO {
    ...
    private <T extends Entity> T findExistingEntity(Entity entity, Class clazz) {
        ...
        if (clazz.equals(Rocket.class)) {
            ...
            Payload pld = (Payload) entity;
            filters.add(new Filter("description", EQUALS, pld.getDescription()))
                    .and(new Filter("mission", EQUALS, pld.getMission()))
                    .and(new Filter("type", EQUALS, pld.getType()))
                    .and(new Filter("weight", EQUALS, pld.getWeight()));
            collection = session.loadAll(Payload.class, filters);
        }
        else if (clazz.equals(RocketFamily.class)) {
            RocketFamily rokFam = (RocketFamily) entity;
            filters.add(new Filter("family", EQUALS, rokFam.getFamily()))
                    .and(new Filter("GTOC mass", EQUALS, rokFam.getPayloadToGTOCapacity()))
                    .and(new Filter("LEOC mass", EQUALS, rokFam.getPayloadToLEOCapacity()))
                    .and(new Filter("subVariation", EQUALS, rokFam.getSubvariation()))
                    .and(new Filter("Variation", EQUALS, rokFam.getVariation()));
            collection = session.loadAll(Payload.class, filters);
        }
